...
<div class="row table-responsive">
    <table class="table table-bordered table-hover">
        ...
        <?php foreach ($page->items as $medicine_stock) { ?>
            <tr<?php if ($medicine_stock->forbidden == 1) { ?> class="danger"<?php } ?>>
            ...
            <td><?= $medicine_stock->manufactory ?></td-->
                ...
                <td><?= $this->tag->linkTo(['medicine_stock/edit/' . $medicine_stock->id, '<i class="fa fa-edit" aria-hidden="true"></i> Edit', 'class' => 'btn btn-xs btn-default']) ?></td>
                <td><?= $this->tag->linkTo(['medicine_stock/delete/' . $medicine_stock->id, '<i class="fa fa-trash-o" aria-hidden="true"></i> Delete', 'class' => 'btn btn-xs btn-danger', 'onClick' => 'return confirm("Are you sure to DELETE?");']) ?></td>
            </tr>
        <?php } ?>
        <?php } ?>
        </tbody>
